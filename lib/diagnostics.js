...
function redactDiagnostic(message, home = os.homedir()) {
  ...
  return text
    ...
    .replace(/([?&](?:api[_-]?key|access[_-]?token|authorization|x-api-key|token|password|secret)=)[^&\s]+/gi, '$1<redacted>')
    .replace(/((?:api[_-]?key|access[_-]?token|authorization|x-api-key|token|password|secret)\s*[:=]\s*)(["']?)[^\s&,}"']+\2/gi, '$1$2<redacted>$2');
}
